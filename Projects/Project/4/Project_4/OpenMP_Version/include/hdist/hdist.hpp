...
namespace hdist {
    ...
    bool calculate(const State &state, Grid &grid, int threadnum) {
        bool stabilized = true;
        bool each_stable[threadnum];

        /* local stable */
        for(int i = 0; i < threadnum; i++){
            each_stable[i] = stabilized;
        }
        ...
        /* join all the local stable */
        for(int i = 0; i < threadnum; i++){
            stabilized &= each_stable[i];
        }
